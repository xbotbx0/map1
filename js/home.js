...
// متغير لتتبع حالة التتبع
let isFollowing = true;

// دالة لتحديث مركز الخريطة عند الحركة
...
document.getElementById('followButton').addEventListener('click', function() {
    ...
    this.innerHTML = isFollowing ? '<i class="fas fa-pause"></i>' : '<i class="fas fa-play"></i>';
});
...
function updateLocation(position) {
    ...
    polyline = L.polyline(path, {color: 'rgb(50, 205, 50)'}).addTo(map);
...
function focusOnLocation() {
    if (currentPosition) { // التحقق مما إذا كان هناك موقع متاح
        map.setView([currentPosition.lat, currentPosition.lng], 16.5); // ضبط الخريطة على الموقع الحالي
...
let isTrackingEnabled = false; // متغير لتتبع حالة التتبع التلقائي
const trackButton = document.querySelector('.nav-button'); // زر التحكم في التتبع

// دالة لتبديل حالة التتبع عند الضغط على الزر
function toggleTracking() {
    isTrackingEnabled = !isTrackingEnabled;
    trackButton.title = isTrackingEnabled ? "إيقاف التتبع" : "تتبع الموقع";

    // تغيير لون الأيقونة عند التفعيل
    const icon = trackButton.querySelector('i');
    if (icon) {
        icon.style.color = isTrackingEnabled ? "red" : "black";
    }
}
